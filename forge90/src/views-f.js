...
const chgMeal = (rid, name, emoji) => { const r = rid && RECIPE[rid]; return rid ? `${esc(r ? r.emoji : emoji || '🍽️')} ${esc(r ? r.name : name || rid)}` : '<span class="muted">nenhum</span>'; };
function syncPanel() {
...
function syncCardHTML() {
  ...
  if (!sy) body = `<p class="small sub" style="margin-top:0">Planeje refeições e faça compras junto com alguém da sua casa que também usa o FORGE 90.</p>${how}
    <form data-form="sync-request" class="grid" style="gap:10px;margin-top:10px"><div class="field"><label>E-mail FORGE 90 da outra pessoa</label><input class="inp" name="email" type="email" required placeholder="partner@example.com" autocomplete="off"></div>
      <div class="field"><label>Refeições para compartilhar</label>${slotBoxes({ breakfast: 1, lunch: 1, dinner: 1, snack1: 1, snack2: 1 })}</div>
      <div><button class="btn primary" type="submit">${icon('users')}Enviar solicitação de sincronização</button></div></form>`;
  else if (sy.status === 'pending' && sy.role === 'requester') body = `<div class="note">${icon('clock')}<span style="flex:1">Aguardando <b>${esc(syncName())}</b> (${esc(sy.partner ? sy.partner.email : '')}) aceitar · enviado ${ago(sy.createdAt)}</span></div>
    <div class="field" style="margin-top:12px"><label>Refeições que você pediu para compartilhar</label>${slotBoxes(sy.slots, true)}</div><div class="row" style="margin-top:12px"><button class="btn" data-act="sync-end">Cancelar solicitação</button></div>`;
  else if (sy.status === 'pending') body = `<div class="note warn">${icon('users')}<span><b>${esc(syncName())}</b> (${esc(sy.partner ? sy.partner.email : '')}) quer sincronizar planos de refeição com você.</span></div>
    <div class="field" style="margin-top:12px"><label>Refeições para compartilhar</label>${slotBoxes(sy.slots, true)}</div>${how}
...
Object.assign(ACT, {
  'sync-open': () => syncPanel(),
  'sync-refresh': async () => { await syncFetch(true); if (SY.snapAt == null && syncActive()) await syncLoadPartner(SY.data.partnerSnapAt); syncPanel(); toast('Atualizado'); },
