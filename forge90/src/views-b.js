...
function viewWorkouts() {
  ...
  const heat = `<table class="tbl heat"><thead><tr><th>Músculo</th>${ALL_PHASES.map(p => `<th style="text-align:center">${p.key === 'test' ? 'Teste' : p.name}</th>`).join('')}</tr></thead><tbody>
...
function viewDiet() {
  ...
  const targets = `<div class="card"><div class="card-h"><h2>Suas metas</h2>${tT.maintMode ? '<span class="pill acc">Meta atingida · manutenção</span>' : ''}<span class="pill">${fmt(cur.w, 1)} lb · ${fmt(cur.bf, 1)}% BF${cur.est ? ' est.' : ''}</span></div>
    ...
    <div class="scroll-x" style="margin-top:14px"><table class="tbl"><tbody>
      ...
      <tr><td>Manutenção — descanso / treino</td><td class="num">${fmt(tR.maint)} / ${fmt(tT.maint)} kcal</td><td class="muted">BMR × ${st.activity} de atividade, + ${st.sessionKcal} kcal nos dias de treino</td></tr>
      <tr><td>${tT.bulking ? 'Superávit' : 'Déficit'}</td><td class="num">${fmt(tT.bulking ? tT.surplus : tT.deficit)} kcal/dia</td><td class="muted">${tT.bulking ? `${fmt(bulkKg(cur.w), 2)} kg/semana` : `${fmt(st.rate, 2)} kg/semana`}${st.kcalAdjust ? ` · ajuste ${st.kcalAdjust > 0 ? '+' : ''}${st.kcalAdjust} kcal` : ''}</td></tr>
    </tbody></table></div>
    <div class="small muted" style="margin-top:14px">As metas e todas as porções do calendário são atualizadas a partir da sua pesagem mais recente.</div></div></div>`;
  const portion = (d, lbl) => { if (!d) return ''; const x = A.days[d]; return `<div class="card" style="box-shadow:none;background:var(--surface-2)"><div class="tiny muted" style="font-weight:700;text-transform:uppercase;letter-spacing:.08em">${lbl} · ${fmtDate(d)}</div>
      <div class="row" style="margin-top:8px;gap:18px"><div><div class="tiny muted">Fontes de proteína</div><div style="font-size:24px;font-weight:700" class="num">×${x.pF.toFixed(2)}</div></div><div><div class="tiny muted">Fontes de carboidratos e gorduras</div><div style="font-size:24px;font-weight:700" class="num">×${x.cF.toFixed(2)}</div></div><div><div class="tiny muted">Total do dia</div><div style="font-size:24px;font-weight:700" class="num">${fmt(x.totals.k)}</div></div></div>
      <a class="btn sm" style="margin-top:10px" href="#/day/${d}">Ver receitas ajustadas ${icon('right')}</a></div>`; };
  const portions = `<div class="card"><div class="card-h"><h2>Sugestões de tamanho das porções</h2></div>
    <div class="small sub" style="margin:-6px 0 12px">Cada receita é escrita com uma porção padrão de 1×. A cada dia, ingredientes proteicos (frango, ovos, iogurte…) são ajustados para atingir sua meta de proteína; depois, carboidratos e gorduras (arroz, batatas, aveia, óleos…) são ajustados para atingir as calorias. Dias de treino recebem mais carboidratos.</div>
    <div class="grid g2" style="gap:12px">${portion(nextT, 'Próximo dia de treino')}${portion(nextR, 'Próximo dia de descanso')}</div>
    ${trend ? `<div class="note ${trend.delta ? 'warn' : 'acc'}" style="margin-top:12px">${icon('trend')}<span>${esc(trend.advice)} ${trend.delta ? `<button class="btn sm" data-act="apply-trend" data-delta="${trend.delta}">Aplicar ${trend.delta > 0 ? '+' : ''}${trend.delta} kcal</button>` : ''}</span></div>` : `<div class="note" style="margin-top:12px">${icon('info')}<span>Depois de ~1–2 semanas de pesagens, o aplicativo compara o que realmente aconteceu na balança com sua meta de ${goalKind() === 'bulk' ? `${fmt(bulkKg(cur.w), 2)} kg/sem de ganho` : goalKind() === 'maintain' ? 'manutenção' : `${fmt(st.rate, 2)} kg/sem de perda`} e sugere um ajuste de calorias.</span></div>`}
    ${goalKind() === 'maintain' ? '' : `<div class="note" style="margin-top:8px">${icon('target')}<span>At ${goalKind() === 'bulk' ? `${fmt(bulkKg(cur.w), 2)} kg/sem` : `${fmt(st.rate, 2)} kg/sem`} você estará perto de <b>${fmt(pj.endW, 0)} kg</b> ${pj.cyc === 1 ? 'no Dia 90' : 'no fim do ciclo ' + pj.cyc}${Math.abs(pj.weeks) > 0.01 ? ` e chegará a ${fmt(st.goalWeight, 1)} kg por volta de <b>${fmtDate(pj.goalDate, { month: 'long', year: 'numeric' })}</b>` : ''}. Mantendo sua massa magra, ${st.goalBF}% BF ≈ <b>${fmt(pj.wAtGoalBF, 0)} kg</b>.</span></div>`}</div>`;
  ...
  return `<div class="page-head"><div class="t"><h1>Plano alimentar</h1><p>${goalLine} As refeições são opções populares ricas em proteínas para preparo antecipado; receitas com várias porções são programadas como sobras para evitar desperdício.</p></div>${syncBtnHTML()}</div>
