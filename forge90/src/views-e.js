...
function admTabHTML() {
  ...
  if (t === 'overview') {
    ...
    return `${st.email.ready ? '' : `<div class="note warn" style="margin-bottom:14px">${icon('mail')}<span><b>O e-mail ainda não está funcionando</b> — convites e mensagens de redefinição de senha não podem ser enviados. Conclua a configuração em <a href="#" data-act="adm-tab" data-v="email">E-mail</a>.</span></div>`}
      ...
      <div class="card"><div class="card-h"><h2>Convites pendentes</h2><span class="pill">${(ADM.invites || []).length}</span><button class="btn sm ghost" style="margin-left:auto" data-act="adm-new">${icon('plus')}Invite</button></div>${(ADM.invites || []).slice(0, 6).map(i => `<div class="sess-row"><span class="avatar">${esc(initials(i.name || i.email))}</span><div style="flex:1;min-width:0"><b>${esc(i.name || i.email)}</b>${i.role === 'admin' ? ' <span class="pill acc">Administrador</span>' : ''}<div class="tiny muted">${i.name ? esc(i.email) + ' · ' : ''}enviado ${ago(i.sentAt)} · ${i.expired ? 'expirado' : 'expira ' + invDue(i).replace(/<[^>]+>/g, '')}</div></div>${invActs(i)}</div>`).join('') || `<div class="muted small">Nenhum convite pendente. Novas contas entram apenas por convite — use <b>Convidar</b> para enviar um link por e-mail (válido por ${st.inviteDays || 7} dias).</div>`}
