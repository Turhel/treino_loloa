...
function navItems() { const n = NAV.slice(); if (isAdmin()) n.push(['admin', 'Administrador', 'shield']); return n; }
...
function renderAuth() {
  ...
  if (s.screen === 'login') body = `<h1>Bem-vindo de volta</h1><p class="sub">Entre para acessar seu plano.</p>${msg}
    ...
    <div class="auth-msg">${icon('info')}<span>Nada depois de alguns minutos? Verifique o spam ou peça ao administrador para enviar pelo menu Administrador → Usuários.</span></div>
    <div class="auth-foot"><a href="#" data-act="auth-go" data-v="login">${icon('left')}Voltar para o login</a></div>`;
...
/* ---------- shared helpers ---------- */
function ago(t) { if (!t) return '—'; const s = Math.round((Date.now() - t) / 1000); if (s < 45) return 'agora mesmo'; if (s < 3600) return Math.round(s / 60) + ' min atrás'; if (s < 86400) return Math.round(s / 3600) + ' h atrás'; if (s < 86400 * 30) return Math.round(s / 86400) + ' d atrás'; return new Date(t).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }); }
const when = t => t ? new Date(t).toLocaleString(undefined, { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }) : '—';
function uaLabel(ua) { ua = ua || ''; const b = /Edg\//.test(ua) ? 'Edge' : /OPR\//.test(ua) ? 'Opera' : /Firefox\//.test(ua) ? 'Firefox' : /Chrome\//.test(ua) ? 'Chrome' : /Safari\//.test(ua) ? 'Safari' : 'Navegador';
...
const serverOnly = title => `<div class="page-head"><div class="t"><h1>${title}</h1></div></div><div class="card"><div class="note">${icon('info')}<span>As contas precisam do servidor FORGE 90. Você abriu o aplicativo como arquivo, então ele está funcionando em modo de usuário único, com os dados salvos neste navegador. Inicie o servidor (consulte o README) e abra <b>http://localhost:8090</b> para entrar.</span></div></div>`;

/* ---------- account settings (#/account) ---------- */
let ACC = null;
function viewAccount() {
  if (AUTH.mode !== 'server' || !AUTH.user) return serverOnly('Conta');
  return `<div class="page-head"><div class="t"><h1>Conta</h1><p>Seu perfil, senha, dispositivos conectados e dados da conta.</p></div>${isAdmin() ? `<a class="btn" href="#/admin">${icon('shield')}Painel administrativo</a>` : ''}</div><div id="acc-root">${ACC ? accountHTML(ACC) : '<div class="card"><div class="auth-spin"></div></div>'}</div>`;
}
...
function accountHTML(d) {
  ...
  const sessions = d.sessions.map(s => `<div class="sess-row"><span class="lk-ic">${icon('device')}</span><div style="flex:1;min-width:0"><b>${esc(uaLabel(s.ua))}</b> ${s.current ? '<span class="pill acc">Este dispositivo</span>' : ''}${s.remember ? '<span class="pill">Mantido conectado</span>' : ''}
      <div class="tiny muted">IP ${esc(s.ip || '—')} · ativo ${ago(s.lastSeen)} · entrou em ${when(s.createdAt)}</div></div>
      <button class="btn sm ${s.current ? 'ghost' : ''}" data-act="acc-revoke" data-id="${s.id}" data-self="${s.current ? 1 : 0}">${s.current ? 'Sair' : 'Desconectar dispositivo'}</button></div>`).join('');
  const events = d.events.map(e => `<div class="ev-row"><span class="ev-dot ${EV_BAD.test(e.type) ? 'bad' : ''}"></span><div style="flex:1;min-width:0"><b>${esc(EV[e.type] || e.type)}</b>${e.detail ? `<span class="tiny muted"> · ${esc(e.detail)}</span>` : ''}<div class="tiny muted">${when(e.t)}${e.ip ? ' · IP ' + esc(e.ip) : ''}</div></div></div>`).join('') || '<div class="muted small">Nenhuma atividade ainda.</div>';
  return `<div class="grid g2">
    <div class="card"><div class="card-h"><h2>${icon('user')}Perfil</h2>${u.owner ? '<span class="pill own">Proprietário</span>' : ''}<span class="pill ${u.role === 'admin' ? 'acc' : ''}">${u.role === 'admin' ? 'Administrador' : 'Membro'}</span></div>
      <div class="acc-id"><div class="acc-pic">${avatarHTML(u, 'xl')}<div class="acc-pic-acts"><label class="btn sm">${icon('upload')}${u.avatarUrl ? 'Alterar foto' : 'Enviar foto'}<input type="file" accept="image/*" data-input="avatar" hidden></label>${u.avatarUrl ? `<button type="button" class="btn sm ghost" data-act="avatar-rm">Remover</button>` : ''}</div></div><div><b>${esc(u.name)}</b>${u.firstName || u.lastName ? `<div class="small">${esc([u.firstName, u.lastName].filter(Boolean).join(' '))}</div>` : ''}<div class="small muted">${esc(u.email)}</div><div class="tiny muted">Membro desde ${new Date(u.createdAt).toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })}</div></div></div>
      <form data-form="acc-profile" class="grid" style="gap:12px;margin-top:14px">
        <div class="grid g2" style="gap:12px"><div class="field"><label>Nome</label><input class="inp" name="firstName" value="${esc(u.firstName || '')}" maxlength="40" autocomplete="given-name"></div><div class="field"><label>Sobrenome</label><input class="inp" name="lastName" value="${esc(u.lastName || '')}" maxlength="40" autocomplete="family-name"></div></div>
        <div class="field"><label>Apelido <span class="muted" style="font-weight:500">— seu nome de exibição</span></label><input class="inp" name="name" value="${esc(u.name)}" maxlength="40" required><span class="tiny muted">Exibido no aplicativo e para qualquer pessoa com quem você sincronizar planos de refeição.</span></div>
        <div class="field"><label>E-mail (usado para login e redefinição de senha)</label><input class="inp" name="email" type="email" value="${esc(u.email)}" required data-input="acc-email"></div>
        <div class="field hidden" id="acc-cur"><label>Senha atual — necessária para alterar seu e-mail</label><input class="inp" name="currentPassword" type="password" autocomplete="current-password"></div>
        <div><button class="btn primary" type="submit">Salvar perfil</button></div></form></div>
    <div class="card"><div class="card-h"><h2>${icon('lock')}Senha</h2><span class="tiny muted">Última alteração ${ago(d.pwChangedAt)}</span></div>
      <form data-form="acc-password" class="grid" style="gap:12px">
        ...
        <div class="row wrap"><button class="btn primary" type="submit">Alterar senha</button><span class="tiny muted">Seus outros dispositivos serão desconectados.</span></div></form>
      <hr class="sep"><label class="row small" style="gap:10px;cursor:pointer"><input type="checkbox" data-input="acc-notify" ${notify.passwordChange !== false ? 'checked' : ''}> Avisar por e-mail quando minha senha for alterada ou redefinida</label>
      <div class="tiny muted" style="margin-top:6px">Esqueceu? Saia da conta e use “Esqueceu a senha?” — o link de redefinição será enviado para ${esc(u.email)} e expira após ${(d.config && d.config.resetMinutes) || 30} minutos.</div></div>
  </div><div style="height:16px"></div>
  <div class="card" id="acc-sync">${syncCardHTML()}</div><div style="height:16px"></div>
  <div class="card"><div class="card-h"><h2>${icon('device')}Dispositivos conectados</h2>${d.sessions.length > 1 ? `<button class="btn sm" data-act="acc-revoke-others">Desconectar todos os outros dispositivos</button>` : ''}</div>${sessions || '<div class="muted small">Nenhuma sessão ativa.</div>'}</div>
  <div style="height:16px"></div>
  <div class="grid g2">
    <div class="card"><div class="card-h"><h2>${icon('activity')}Atividade recente</h2></div><div class="ev-list">${events}</div></div>
    <div class="card"><div class="card-h"><h2>${icon('download')}Seus dados</h2></div>
      <div class="small sub">Seu plano, pesagens, registros de força, alimentos e receitas personalizados ficam salvos na sua conta e acompanham você em qualquer dispositivo em que entrar.</div>
      <div class="grid g2" style="gap:10px;margin-top:12px"><div class="ms-stat"><span class="tiny muted">Último salvamento</span><b style="font-size:16px">${ago(AUTH.savedAt || d.data.updatedAt)}</b></div><div class="ms-stat"><span class="tiny muted">Tamanho</span><b style="font-size:16px">${kb(d.data.bytes)}</b></div></div>
      <div class="row wrap" style="margin-top:12px"><button class="btn" data-act="export">${icon('download')}Exportar backup</button><label class="btn">${icon('upload')}Importar backup<input type="file" accept="application/json" data-input="import" hidden></label><a class="btn ghost" href="#/settings">Configurações do plano ${icon('right')}</a></div>
      <hr class="sep"><div class="danger-zone"><div><b>Excluir conta</b><div class="tiny muted">Remove permanentemente sua conta e todos os dados. Esta ação não pode ser desfeita.</div></div><button class="btn danger" data-act="acc-delete">${icon('trash')}Excluir conta</button></div></div>
  </div>`;
}
async function accSubmit(form) {
  ...
  try {
    ...
    else { if (fd.password !== fd.confirm) throw new Error('As duas novas senhas não coincidem.'); const r = await api('POST', '/api/account/password', { current: fd.current, next: fd.password }); AUTH.user = r.user; toast(`Senha alterada${r.signedOut ? ` — ${r.signedOut} outro${r.signedOut === 1 ? '' : 's'} dispositivo${r.signedOut === 1 ? '' : 's'} desconectado${r.signedOut === 1 ? '' : 's'}` : ''}`); }
    await accountAfter();
...
Object.assign(ACT, {
  ...
  'acc-delete': () => { modal(`<h2>Excluir sua conta?</h2><p class="sub small">Isso exclui permanentemente <b>${esc(AUTH.user.email)}</b>, incluindo plano, pesagens, registros, alimentos e receitas personalizados. Exporte um backup antes se quiser guardar alguma coisa.</p>
      <form data-form="acc-del" style="margin-top:12px">${pwField('password', 'Sua senha', 'current-password')}<div class="row" style="justify-content:flex-end;margin-top:14px"><button type="button" class="btn" data-act="close-modal">Cancelar</button><button class="btn danger" type="submit">${icon('trash')}Excluir para sempre</button></div></form>`, 'sm'); }
});
...
function viewAdmin() {
  if (AUTH.mode !== 'server' || !AUTH.user) return serverOnly('Administrador');
  if (!isAdmin()) return `<div class="page-head"><div class="t"><h1>Administrador</h1></div></div><div class="card"><div class="note warn">${icon('lock')}<span>Somente administradores podem abrir o painel administrativo.</span></div></div>`;
...
function admTabHTML() {
  ...
  if (t === 'overview') {
    ...
    return `${st.email.ready ? '' : `<div class="note warn" style="margin-bottom:14px">${icon('mail')}<span><b>O e-mail ainda não está funcionando</b> — convites e mensagens de redefinição de senha não podem ser enviados. Conclua a configuração em <a href="#" data-act="adm-tab" data-v="email">E-mail</a>.</span></div>`}
      <div class="grid g4" style="gap:10px">${tile('Contas', s.users, `${s.admins} administrador${s.admins === 1 ? '' : 'es'} · ${s.invites || 0} convite${s.invites === 1 ? '' : 's'} pendente${s.invites === 1 ? '' : 's'}`)}${tile('Sessões conectadas', s.sessions, 'ativas agora ou mantidas conectadas')}${tile('Logins · 24 h', s.logins24, `${s.failed24} tentativa${s.failed24 === 1 ? '' : 's'} com falha`, s.failed24 > 10)}${tile('E-mails de redefinição · 24 h', s.emails24, s.emailFail24 ? `${s.emailFail24} falha${s.emailFail24 === 1 ? '' : 's'} no envio` : 'nenhuma falha', s.emailFail24 > 0)}</div>
      <div style="height:16px"></div><div class="grid g2">
      <div class="card"><div class="card-h"><h2>Convites pendentes</h2><span class="pill">${(ADM.invites || []).length}</span><button class="btn sm ghost" style="margin-left:auto" data-act="adm-new">${icon('plus')}Convidar</button></div>${(ADM.invites || []).slice(0, 6).map(i => `<div class="sess-row"><span class="avatar">${esc(initials(i.name || i.email))}</span><div style="flex:1;min-width:0"><b>${esc(i.name || i.email)}</b>${i.role === 'admin' ? ' <span class="pill acc">Administrador</span>' : ''}<div class="tiny muted">${i.name ? esc(i.email) + ' · ' : ''}enviado ${ago(i.sentAt)} · ${i.expired ? 'expirado' : 'expira ' + invDue(i).replace(/<[^>]+>/g, '')}</div></div>${invActs(i)}</div>`).join('') || `<div class="muted small">Nenhum convite pendente. Novas contas entram apenas por convite — use <b>Convidar</b> para enviar um link por e-mail (válido por ${st.inviteDays || 7} dias).</div>`}
      ...
      <div class="card"><div class="card-h"><h2>Contas bloqueadas</h2><span class="pill">${locked.length}</span></div>${locked.map(u => `<div class="sess-row">${avatarHTML(u)}<div style="flex:1;min-width:0"><b>${esc(u.name)}</b><div class="tiny muted">${esc(u.email)} · até ${when(u.lockedUntil)}</div></div><button class="btn sm" data-act="adm-unlock" data-id="${u.id}">Desbloquear</button></div>`).join('') || `<div class="muted small">Nenhuma conta bloqueada. As contas são bloqueadas após ${st.security.lockThreshold} tentativas de login com falha por ${st.security.lockMinutes} minutos${st.security.autoResetOnLock ? ', e o proprietário recebe um link de redefinição por e-mail' : ''}.</div>`}</div></div>
      <div style="height:16px"></div><div class="card"><div class="card-h"><h2>Configurações rápidas</h2></div><div class="grid g3" style="gap:10px">
        ${admSwitch('Enviar link de redefinição quando houver bloqueio', 'security.autoResetOnLock', st.security.autoResetOnLock)}${admSwitch('Avisar por e-mail quando a senha for alterada', 'security.notifyPasswordChange', st.security.notifyPasswordChange)}</div></div>`;
  }
  if (t === 'users') {
    ...
    const rows = list.map(u => { const me = u.id === AUTH.user.id;
      return `<tr><td><div class="row" style="gap:10px;flex-wrap:nowrap">${avatarHTML(u)}<div style="min-width:0"><b>${esc(u.name)}</b>${me ? ' <span class="pill">Você</span>' : ''}<div class="tiny muted">${esc(u.email)}</div></div></div></td>
        <td>${u.owner ? `<span class="pill own" title="Pessoa que configurou este servidor">Proprietário</span>` : `<label class="adm-role" title="${me ? 'Você não pode remover seu próprio acesso de administrador aqui' : !(AUTH.user && AUTH.user.owner) ? 'Somente o proprietário pode alterar o acesso de administrador' : 'Conceder ou remover privilégios de administrador'}"><input type="checkbox" data-input="adm-role" data-id="${u.id}" ${u.role === 'admin' ? 'checked' : ''} ${me || u.status !== 'active' || !(AUTH.user && AUTH.user.owner) ? 'disabled' : ''}><i class="switch ${u.role === 'admin' ? 'on' : ''}" aria-hidden="true"><i></i></i><span class="small">${u.role === 'admin' ? 'Administrador' : 'Membro'}</span></label>`}</td>
        <td>${statusPill(u)}${u.mustChange ? ' <span class="pill" title="Deve escolher uma nova senha no próximo login">Senha temporária</span>' : ''}</td>
        ...
        <td style="text-align:right;white-space:nowrap">${u.status === 'pending' ? `<button class="btn sm primary" data-act="adm-approve" data-id="${u.id}">Aprovar</button> ` : ''}${u.locked ? `<button class="btn sm" data-act="adm-unlock" data-id="${u.id}">Desbloquear</button> ` : ''}<button class="btn sm" data-act="adm-user" data-id="${u.id}">Gerenciar</button></td></tr>`; }).join('');
    const invs = ADM.invites || [];
    const invCard = invs.length ? `<div class="card" style="margin-bottom:16px"><div class="card-h"><h2>Convites pendentes</h2><span class="pill">${invs.length}</span><span class="tiny muted" style="margin-left:auto">Os links duram ${(ADM.settings && ADM.settings.inviteDays) || 7} dias e funcionam uma vez</span></div><div class="scroll-x"><table class="tbl adm-invites"><thead><tr><th>Convidado</th><th>Função</th><th>Enviado</th><th>Expira</th><th></th></tr></thead><tbody>${invs.map(i => `<tr><td><div class="row" style="gap:10px;flex-wrap:nowrap"><span class="avatar">${esc(initials(i.name || i.email))}</span><div style="min-width:0"><b>${esc(i.name || i.email)}</b>${i.name ? `<div class="tiny muted">${esc(i.email)}</div>` : ''}</div></div></td><td><span class="pill ${i.role === 'admin' ? 'acc' : ''}">${i.role === 'admin' ? 'Administrador' : 'Membro'}</span></td><td class="small">${ago(i.sentAt)}${i.invitedBy ? `<div class="tiny muted">por ${esc(i.invitedBy)}${i.sends > 1 ? ` · enviado ${i.sends}×` : ''}</div>` : ''}</td><td>${invDue(i)}</td><td style="text-align:right;white-space:nowrap">${invActs(i)}</td></tr>`).join('')}</tbody></table></div></div>` : '';
    return `${invCard}<div class="card"><div class="row wrap" style="margin-bottom:12px"><input class="inp" style="max-width:260px" placeholder="Buscar nome ou e-mail…" data-input="adm-q" value="${esc(ADM.q)}" aria-label="Buscar usuários">
        <div class="filters">${[['all', 'Todos'], ['admin', 'Administradores']].concat(counts.pending ? [['pending', 'Aguardando aprovação']] : [], [['locked', 'Bloqueados'], ['disabled', 'Desativados']]).map(([k, l]) => `<button class="${ADM.filter === k ? 'on' : ''}" data-act="adm-filter" data-v="${k}">${l} ${counts[k]}</button>`).join('')}</div>
        <button class="btn primary" style="margin-left:auto" data-act="adm-new">${icon('mail')}Convidar usuário</button></div>
      <div class="scroll-x"><table class="tbl adm-users"><thead><tr><th>Usuário</th><th>Função</th><th>Status</th><th>Último acesso</th><th>Dispositivos</th><th>Dados</th><th></th></tr></thead><tbody>${rows || '<tr><td colspan="7" class="muted">Nenhum usuário corresponde à busca.</td></tr>'}</tbody></table></div></div>`;
  }
  ...
  if (t === 'app') {
    return `<form data-form="adm-app" class="card"><div class="card-h"><h2>Configurações do aplicativo</h2></div>
      ...
      <div class="field"><label>Tema padrão para novas contas</label><select class="inp" name="theme">${[['dark','Escuro'],['light','Claro'],['system','Sistema']].map(([v,l]) => `<option value="${v}" ${st.defaults.theme === v ? 'selected' : ''}>${l}</option>`).join('')}</select></div>
  ...
  if (t === 'email') { const e = st.email;
    ...
    return `<div class="grid g-split"><form data-form="adm-email" class="card"><div class="card-h"><h2>E-mail de saída (SMTP)</h2>${e.ready ? '<span class="pill acc">Pronto</span>' : '<span class="pill warn-pill">Não configurado</span>'}</div>${warn}<div class="grid g2" style="gap:14px">
      ...
      <div><div class="card"><div class="card-h"><h2>Enviar um teste</h2></div><div class="row" style="gap:6px;flex-wrap:nowrap"><input class="inp" id="adm-test-to" type="email" value="${esc(AUTH.user.email)}" aria-label="Enviar teste para"><button class="btn" data-act="adm-test-email">${icon('mail')}Enviar</button></div><div class="tiny muted" style="margin-top:8px" id="adm-test-out">Envia uma mensagem de exemplo usando as configurações salvas.</div></div>
  ...
  if (t === 'data') { if (!ADM.stats) return spin; const s = ADM.stats;
    return `<div class="grid g2"><div class="card"><div class="card-h"><h2>Backup completo</h2></div><div class="small sub">Baixa todas as contas (sem senhas), os dados de plano de cada conta e as configurações do aplicativo (sem a senha do e-mail) em um único arquivo JSON.</div>
      <div class="row" style="margin-top:12px"><button class="btn primary" data-act="adm-backup">${icon('download')}Baixar backup</button></div>
...
function admProxyHTML() {
  ...
  else add('warn', 'HTTP sem criptografia', `Você está conectado diretamente a ${code(reqHost)} sem criptografia, então as senhas trafegam pela rede em texto simples. Use seu endereço https://.`);
  ...
  if (!p.appUrl) add('warn', 'Endereço do aplicativo', 'Não definido; por isso, links de convite e redefinição usam o endereço de origem de cada solicitação. Defina <code>APP_URL</code> (ou Administrador → Configurações do aplicativo) com seu endereço https://.');
  ...
  else if (!rh.setting) add('warn', 'Exigir HTTPS', 'Desativado (Administrador → Segurança), então acessos por IP usando HTTP sem criptografia são permitidos.');
...
async function admSave(patch, msg) { try { ADM.settings = await api('PATCH', '/api/admin/settings', patch); AUTH.config = Object.assign({}, AUTH.config, { appName: ADM.settings.appName }); document.title = appTitle() + ' · Administrador'; toast(msg || 'Configurações salvas'); const r = $('#adm-root'); if (r) r.innerHTML = admTabHTML(); } catch (e) { toast(e.message); } }
...
function admUserModal(id) {
  ...
  modal(`<div class="adm-user-modal"><div class="row" style="align-items:flex-start">${avatarHTML(u, 'lg')}<div style="flex:1;min-width:0"><h2>${esc(u.name)}${me ? ' <span class="pill">Você</span>' : ''}</h2><div class="small muted">${esc(u.email)}</div><div class="row wrap" style="gap:6px;margin-top:6px">${u.owner ? '<span class="pill own">Proprietário</span>' : ''}<span class="pill ${u.role === 'admin' ? 'acc' : ''}">${u.role === 'admin' ? 'Administrador' : 'Membro'}</span>${statusPill(u)}</div></div><button class="btn icon ghost" data-act="close-modal">${icon('x')}</button></div>
    <div class="grid g3 adm-info">${info.map(([k, v]) => `<div><span class="tiny muted">${k}</span><b class="small">${v}</b></div>`).join('')}</div>
    ${locked ? `<div class="note" style="margin-top:12px">${icon('shield')}<span>Esta é a conta do proprietário, a pessoa que configurou o servidor. Somente ela pode alterar esta conta.</span></div>` : ''}
    <h3>Acesso</h3><div class="adm-acts">
      ${u.status === 'pending' ? `<button class="btn primary" data-act="adm-approve" data-id="${id}"${why}>${icon('check')}Aprovar conta</button>` : ''}
      ${me || u.owner ? '' : u.role === 'admin' ? `<button class="btn" data-act="adm-setrole" data-id="${id}" data-v="user"${iAmOwner ? '' : ' disabled title="Somente o proprietário pode remover o acesso de administrador"'}>${icon('shield')}Remover privilégios de administrador</button>` : `<button class="btn" data-act="adm-setrole" data-id="${id}" data-v="admin" ${u.status !== 'active' ? 'disabled title="Ative a conta primeiro"' : iAmOwner ? '' : 'disabled title="Somente o proprietário pode tornar alguém administrador"'}>${icon('shield')}Tornar administrador</button>`}
      ${me || locked ? '' : u.status === 'disabled' ? `<button class="btn" data-act="adm-status" data-id="${id}" data-v="active">Ativar conta</button>` : u.status === 'active' ? `<button class="btn" data-act="adm-status" data-id="${id}" data-v="disabled"${whyAdm}>Desativar conta</button>` : ''}</div>
    ${u.avatarUrl && !me && !locked ? `<h3>Foto de perfil</h3><div class="adm-acts"><button class="btn" data-act="adm-avatar-rm" data-id="${id}">${icon('trash')}Remover foto de perfil</button></div>` : ''}
    <h3>Ajuda para login</h3><div class="adm-acts">
      ${u.locked || u.failed ? `<button class="btn" data-act="adm-unlock" data-id="${id}"${why}>${icon('lock')}Desbloquear${u.locked ? '' : ' / limpar tentativas com falha'}</button>` : ''}
      <button class="btn" data-act="adm-reset" data-id="${id}" ${u.status === 'disabled' ? 'disabled' : ''}${why}>${icon('mail')}Enviar link de redefinição por e-mail</button>
      <button class="btn" data-act="adm-temp" data-id="${id}"${why || whyAdm}>${icon('key')}Definir senha temporária</button>
      <button class="btn" data-act="adm-signout" data-id="${id}" ${u.sessions ? '' : 'disabled'}${why || whyAdm}>${icon('logout')}Desconectar de todos os dispositivos</button></div>
    <h3>Perfil e dados</h3><form data-form="adm-profile" data-id="${id}" class="grid g2" style="gap:10px"><div class="field"><label>Nome</label><input class="inp" name="name" value="${esc(u.name)}"${locked ? ' disabled' : ''}></div><div class="field"><label>E-mail</label><input class="inp" name="email" type="email" value="${esc(u.email)}"${locked ? ' disabled' : ''}></div>
      <div class="row wrap" style="grid-column:1/-1"><button class="btn" type="submit"${why}>Salvar perfil</button><button class="btn ghost" type="button" data-act="adm-userdata" data-id="${id}" ${u.dataBytes ? '' : 'disabled'}${why}>${icon('download')}Baixar dados do plano</button>
      ${me || u.owner ? '' : `<button class="btn danger" type="button" data-act="adm-delete" data-id="${id}" style="margin-left:auto"${whyAdm}>${icon('trash')}Excluir conta</button>`}</div></form></div>`);
}
function admNewUser() {
  const st = ADM.settings; const ready = !st || st.email.ready;
  modal(`<div class="row"><h2 style="flex:1">Convidar alguém</h2><button class="btn icon ghost" data-act="close-modal">${icon('x')}</button></div>
    <p class="sub small" style="margin:6px 0 0">A pessoa receberá um e-mail com um link para criar a conta e escolher uma senha. O link é válido por <b>${(st && st.inviteDays) || 7} dias</b> e funciona uma única vez.</p>
    ${ready ? '' : `<div class="note warn" style="margin-top:12px">${icon('mail')}<span>O e-mail ainda não está configurado, então os convites não podem ser enviados. <a href="#" data-act="adm-tab" data-v="email">Configure o e-mail</a> primeiro.</span></div>`}
    <form data-form="adm-new" class="grid g2" style="gap:12px;margin-top:12px"><div class="field" style="grid-column:1/-1"><label>E-mail</label><input class="inp" name="email" type="email" required autocomplete="off" placeholder="name@example.com"></div>
      <div class="field"><label>Nome <span class="muted" style="font-weight:500">— opcional</span></label><input class="inp" name="name" maxlength="80" placeholder="A pessoa poderá alterá-la"></div>
      <div class="field"><label>Função</label><select class="inp" name="role"><option value="user">Membro</option><option value="admin">Administrador</option></select></div>
      <div class="row" style="grid-column:1/-1;justify-content:flex-end"><button type="button" class="btn" data-act="close-modal">Cancelar</button><button class="btn primary" type="submit" ${ready ? '' : 'disabled'}>${icon('mail')}Enviar convite</button></div></form>`);
...
Object.assign(ACT, {
  ...
  'adm-inv-revoke': el => { const i = (ADM.invites || []).find(x => x.id === el.dataset.id); if (!i) return; confirmBox('Revogar este convite?', `O link enviado para <b>${esc(i.email)}</b> deixará de funcionar. Você poderá convidar essa pessoa novamente depois.`, 'Revogar convite', () => admInvAction(i.id, 'DELETE', '', 'Convite revogado'), true); },
  'adm-user': el => admUserModal(el.dataset.id),
  'adm-avatar-rm': el => confirmBox('Remover foto do perfil?', 'A foto será excluída do servidor. A pessoa poderá enviar uma nova quando quiser.', 'Remover', () => admUserAction(el.dataset.id, 'DELETE', '/avatar', undefined, 'Foto do perfil removida'), true),
  'adm-approve': el => admUserAction(el.dataset.id, 'PATCH', '', { status: 'active' }, 'Conta aprovada — a pessoa foi avisada por e-mail'),
  'adm-decline': el => { const u = ADM.users.find(x => x.id === el.dataset.id); confirmBox('Recusar este cadastro?', `Excluir a conta pendente de <b>${esc(u.email)}</b>?`, 'Recusar', () => admUserAction(u.id, 'DELETE', '', undefined, 'Cadastro recusado'), true); },
  ...
  'adm-reset': el => { const u = ADM.users.find(x => x.id === el.dataset.id); confirmBox('Enviar link de redefinição por e-mail?', `Enviar para <b>${esc(u.email)}</b> um link de redefinição de senha que expira em ${ADM.settings ? ADM.settings.resetMinutes : 30} minutos?`, 'Enviar e-mail', () => admUserAction(u.id, 'POST', '/send-reset', undefined, 'E-mail de redefinição enviado')); },
