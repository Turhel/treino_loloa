...
function accountHTML(d) {
  ...
  return `<div class="grid g2">
    <div class="card"><div class="card-h"><h2>${icon('user')}Profile</h2>${u.owner ? '<span class="pill own">Proprietário</span>' : ''}<span class="pill ${u.role === 'admin' ? 'acc' : ''}">${u.role === 'admin' ? 'Administrador' : 'Membro'}</span></div>
  ...
  <div class="grid g2">
    ...
    <div class="card"><div class="card-h"><h2>${icon('download')}Your data</h2></div>
      ...
      <hr class="sep"><div class="danger-zone"><div><b>Delete account</b><div class="tiny muted">Permanently removes your account and all of its data. This can’t be undone.</div></div><button class="btn danger" data-act="acc-delete">${icon('trash')}Excluir conta</button></div></div>
  </div>`;
}
...
Object.assign(ACT, {
  ...
  'acc-delete': () => { modal(`<h2>Delete your account?</h2><p class="sub small">This permanently deletes <b>${esc(AUTH.user.email)}</b> with its plan, weigh-ins, logs, custom foods and recipes. Export a backup first if you want to keep anything.</p>
      <form data-form="acc-del" style="margin-top:12px">${pwField('password', 'Sua senha', 'current-password')}<div class="row" style="justify-content:flex-end;margin-top:14px"><button type="button" class="btn" data-act="close-modal">Cancelar</button><button class="btn danger" type="submit">${icon('trash')}Delete forever</button></div></form>`, 'sm'); }
});
