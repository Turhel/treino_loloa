...
function admTabHTML() {
  ...
  if (t === 'users') {
    ...
    return `${invCard}<div class="card"><div class="row wrap" style="margin-bottom:12px"><input class="inp" style="max-width:260px" placeholder="Search name or email…" data-input="adm-q" value="${esc(ADM.q)}" aria-label="Search users">
      ...
      <div class="scroll-x"><table class="tbl adm-users"><thead><tr><th>User</th><th>Role</th><th>Status</th><th>Último acesso</th><th>Devices</th><th>Data</th><th></th></tr></thead><tbody>${rows || '<tr><td colspan="7" class="muted">No users match.</td></tr>'}</tbody></table></div></div>`;
  }
  ...
  if (t === 'security') { const s = st.security;
    return `<form data-form="adm-security" class="card"><div class="card-h"><h2>Sign-in security</h2></div><div class="grid g2" style="gap:14px">
      ${admNum('Tamanho mínimo da senha', 'pwMinLength', s.pwMinLength, 8, 64, 'characters')}${admChk('Require letters and a number or symbol', 'pwRequireMix', s.pwRequireMix)}
      ${admNum('Falha ao entrars before lockout', 'lockThreshold', s.lockThreshold, 3, 20, 'attempts')}${admNum('Lockout lasts', 'lockMinutes', s.lockMinutes, 1, 1440, 'minutes')}
  ...
  if (t === 'app') {
    return `<form data-form="adm-app" class="card"><div class="card-h"><h2>Configurações do aplicativo</h2></div>
      <div class="note acc" style="margin-bottom:14px">${icon('mail')}<span><b>New accounts are invite-only.</b> Um administrador invites people from <a href="#" data-act="adm-tab" data-v="users">Users → Invite user</a>. Each invite link lasts ${st.inviteDays || 7} days and works once; nobody can create an account on their own.</span></div>
