...
function viewPantry() {
  ...
  const foodRow = f => `<div class="pan-food"><div class="pan-name"><b>${esc(pantryName(f))}</b>${ING[f] && ING[f].brand ? `<span class="tiny muted">${esc(ING[f].brand)}</span>` : ''}<span class="tiny muted">${esc(pantryQtyText(f, pantryHave(f, items)))} no total${flat && ING[f] ? ' · ' + esc(pt(ING[f].a)) : ''}</span></div><div class="pan-lots">${byFood[f].sort((a, b) => (a.exp || '9999') < (b.exp || '9999') ? -1 : 1).map(lotHTML).join('')}</div></div>`;
  let list;
  if (!flat) { const aisles = {}; foods.forEach(f => { const a = ING[f] ? ING[f].a : 'Pantry'; (aisles[a] = aisles[a] || []).push(f); });
    list = AISLES.concat(Object.keys(aisles).filter(a => !AISLES.includes(a))).filter(a => aisles[a]).map(a => `<div class="pan-aisle"><h3>${esc(pt(a))}</h3>${aisles[a].sort(byName).map(foodRow).join('')}</div>`).join(''); }
  ...
  const soonCard = soon.length ? `<div class="card pan-soon"><div class="card-h"><h2>${icon('clock')}Vencendo em breve</h2><span class="pill warn-pill">${soon.length}</span></div>
      ...
      <div class="tiny muted" style="margin-top:6px">Itens com validade nos próximos ${PANTRY_SOON} dias aparecem aqui.</div></div>` : '';
  return `<div class="page-head"><div class="t"><h1>Despensa</h1><p>O que você já tem em casa. A lista de compras marca o que está disponível, e as refeições planejadas descontam os itens da despensa automaticamente conforme os dias passam${shared ? ` — compartilhada com ${esc(syncName())}` : ''}.</p></div>
    ...
    <div class="card"><div class="card-h"><h2>Na despensa</h2><span class="pill">${foods.length < all.length ? `${foods.length} de ${all.length}` : all.length} alimento${all.length === 1 ? '' : 's'}</span>${shared ? `<span class="pill acc">${icon('users')}Compartilhada</span>` : ''}</div>
