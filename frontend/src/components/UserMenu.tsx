...
export function UserMenu({ user, syncStatus, onOpenAuth, onSignOut, onSyncNow }: Props) {
  ...
  return (
    <div className="relative flex items-center gap-2">
      <span title={syncStatus} className={`hidden items-center rounded-full px-3 py-1 text-xs font-black ring-1 md:inline-flex ${statusClass}`}>{syncStatus}</span>
      ...
      {user ? (
        <details className="group relative">
          <summary className="list-none">
            <span className="inline-flex h-10 cursor-pointer items-center gap-2 rounded-xl border border-zinc-800 bg-zinc-900 px-3 text-sm font-black text-zinc-200 transition hover:bg-zinc-800">
              <UserCircle className="h-4 w-4" />
              <span className="hidden max-w-[150px] truncate lg:inline">{user.email}</span>
            </span>
          </summary>
          <div className="absolute right-0 z-50 mt-3 w-64 rounded-2xl border border-zinc-800 bg-zinc-950 p-2 shadow-2xl shadow-black/50">
        ...
        <button onClick={onOpenAuth} className="inline-flex h-10 items-center gap-2 rounded-xl bg-zinc-100 px-3 text-sm font-black text-zinc-950 transition hover:bg-white"><UserCircle className="h-4 w-4" /><span className="hidden sm:inline">Entrar / Criar conta</span><span className="sm:hidden">Entrar</span></button>
      )}
    </div>
  );
}
